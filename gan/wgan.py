...
class DCGAN():
    def __init__(self):
        ...
        self.discriminator.compile(loss=self.wasserstein_loss, optimizer=Adam(learning_rate=0.00005), metrics=['accuracy'])
        ...
        self.combined_model.compile(loss=self.wasserstein_loss, optimizer=Adam(learning_rate=0.00005), metrics=['accuracy'])

    def wasserstein_loss(self, real_y, predicted_y):
        return -backend.mean(real_y*predicted_y)
    ...
    def build_discriminator(self):
        ...
        x = Flatten()(x)
        d_output = Dense(units=1)(x)

        return Model(d_input, d_output)

    def train_discriminator(self, X_train, batch_size, clip_threshold=0.01):
        # Create labels
        ...
        for layer in self.discriminator.layers:
            ...
            weights = [np.clip(weight, -clip_threshold, clip_threshold) for weight in weights]
